...
func TestFindFreeLocationSkipsUnhealthyDisk(t *testing.T) {
	...
	// Use locations without the background recovery loop so markUnhealthy
	// is not raced by tryRecoverHealth from NewDiskLocation's goroutine.
	locGood := newHealthTestLocation(dirGood)
	locBad := newHealthTestLocation(dirBad)
	locGood.MaxVolumeCount = 8
	locBad.MaxVolumeCount = 8
	defer locGood.Close()
	defer locBad.Close()
...
func TestStartupUnhealthyMountPoint(t *testing.T) {
	...
	loc := NewDiskLocation(dir, 4, util.MinFreeSpace{Type: util.AsPercent, Percent: 1, Raw: "1"}, idx, types.HardDriveType, nil, stats.DiskIOProbeConfig{})
	defer loc.Close()
...
func TestAddVolumeReportsDiskError(t *testing.T) {
	...
	err := s.addVolume(1, "", NeedleMapInMemory, nil, nil, 0, needle.GetCurrentVersion(), 0, types.HardDriveType, 0)
	if err == nil {
...
func TestUnhealthyDirMarksExistingVolumesReadOnly(t *testing.T) {
	...
	locGood := NewDiskLocation(dirGood, 8, util.MinFreeSpace{Type: util.AsPercent, Percent: 1, Raw: "1"}, idxGood, types.HardDriveType, nil, stats.DiskIOProbeConfig{})
	locBad := NewDiskLocation(dirBad, 8, util.MinFreeSpace{Type: util.AsPercent, Percent: 1, Raw: "1"}, idxBad, types.HardDriveType, nil, stats.DiskIOProbeConfig{})
	defer locGood.Close()
	defer locBad.Close()
	...
	if err := s.AddVolume(1, "", NeedleMapInMemory, "000", "", 0, needle.GetCurrentVersion(), 0, types.HardDriveType, 0); err != nil {
		t.Fatal(err)
	}
	vol := s.findVolume(1)
	if vol == nil {
...
func TestHeartbeatReportsUnhealthyDirVolumesReadOnly(t *testing.T) {
	...
	locData1 := NewDiskLocation(dirData1, 8, minFree, idx1, types.HardDriveType, nil, stats.DiskIOProbeConfig{})
	locData2 := NewDiskLocation(dirData2, 8, minFree, idx2, types.HardDriveType, nil, stats.DiskIOProbeConfig{})
	...
	if err := s.AddVolume(2, "", NeedleMapInMemory, "000", "", 0, needle.GetCurrentVersion(), 0, types.HardDriveType, 0); err != nil {
		t.Fatal(err)
	}
