...
func (s *Store) AddDiskLocation(dir string, maxVolumeCount int32, minFreeSpace util.MinFreeSpace, diskType types.DiskType, ldbTimeout int64) error {
	...
	location, err := NewDiskLocationOrError(dir, maxVolumeCount, minFreeSpace, "", diskType, nil, s.diskProbeConfig)
	if err != nil {
		return err
	}
	...
	diskId := uint32(len(s.Locations))
	location.loadExistingVolumesWithId(s.NeedleMapKind, ldbTimeout, diskId)
